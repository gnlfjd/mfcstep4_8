...
BOOL CgPrjDlg::OnInitDialog()
{
	...
	//MoveWindow(0, 0, 1280, 580);
	MoveWindow(0, 0, 645, 580);
...
void CgPrjDlg::OnBnClickedButton1()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	int RSize;
	RSize = GetDlgItemInt(IDC_EDT_NUM);
	...
	{
		CString str;
		str.Format(_T("원의 사이즈를 0 ~ %d 까지 입력해주세요."), CheckSize);
		MessageBox(str);
		return;
	}
	...
	//원이 사이즈 밖으로 벗어나면 다시 랜덤 받기
	while (true)
	{
		if ((nWidth < x + RSize * 2) || (nHeight < y + RSize * 2))
		{
			x = rand() % nWidth;
			y = rand() % nHeight;
		}
	...
	m_pDlgImage->SetCData(x, y, RSize);
	m_pDlgImage->GetdrawCircle();
	GetCenterData();
	m_pDlgImage->Invalidate();
...
void CgPrjDlg::GetCenterData()
{
	...
	//cout << "CenterX : " << dCenterX << " CenterY : " << dCenterY << endl;

	int x = round(dCenterX);
	int y = round(dCenterY);

	cout << "CenterX : " << x << " CenterY : " << y << endl;
	
	m_pDlgImage->SetCenterData(x, y);
